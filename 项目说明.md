...
## 🔌 v7.5.16 版本更新 - 新增端口协议握手功率显示功能

**最新更新（v7.5.16）**：新增端口协议握手功率显示功能，在UI2系统的端口详细页面中显示各端口的协议握手功率。系统能够智能计算PD协议握手功率，优先使用operating参数，同时支持多种快充协议的典型功率显示，提升用户对充电协商过程的了解。

### v7.5.16 关键功能
- 🔌 **协议握手功率显示**：在UI2系统端口详细页面新增协议握手功率显示组件
- ⚡ **智能功率计算**：优先使用PD协议的operating_voltage和operating_current参数计算真实握手功率
- 📊 **多协议支持**：支持QC2.0/3.0、PD 3.0/3.1、VOOC、AFC、FCP等多种快充协议的功率显示
- 🎯 **动态功率推算**：根据协议类型和当前电压智能推算典型握手功率值
- 🔧 **数据结构优化**：在PowerMonitorData中新增protocol_handshake_power字段

### 协议握手功率计算逻辑
- **PD协议**：优先使用operating_voltage × operating_current计算实际握手功率
- **QC2.0**：18W典型功率
- **QC3.0**：27W典型功率
- **PD 3.0**：根据电压档位计算（5V=15W, 9V=27W, 15V=45W, 20V=65W）
- **PD 3.1**：100W-140W高功率支持
- **VOOC/SVOOC**：20W-65W私有快充协议
- **其他协议**：使用当前实际功率作为参考

### 显示效果
- 在UI2系统的端口详细页面中，"功率："标签下方显示协议握手功率
- 显示格式：直接显示功率数值，如"65.0W"、"27.0W"等
- 无效或未知协议时显示"--W"

## 🔄 v7.5.15 版本更新 - 优化WiFi重连机制和AP模式智能重连策略

**更新（v7.5.15）**：全面优化WiFi重连机制，解决WiFi连接中断后无法自动重连的问题。新增AP模式下的智能重连策略，只在保存了WiFi配置的情况下进行重连尝试，避免资源浪费，提升系统稳定性和用户体验。

### v7.5.15 关键优化
- 🔄 **WiFi重连机制重构**：修复AP模式启动后不再尝试重连WiFi的问题
- ⏰ **重连参数优化**：重连间隔从1分钟调整为2分钟，最大尝试次数从5次增至10次
- 🎯 **智能重连策略**：AP模式下只有在保存了WiFi配置信息时才进行重连尝试
- 🔧 **资源优化**：避免在无WiFi配置时进行无意义的重连尝试，减少资源消耗
- ⚡ **配置检测机制**：当用户通过AP模式新增WiFi配置后，自动检测并开始重连尝试
- 🛡️ **冷却期调整**：达到最大重连次数后的等待时间从5分钟调整为10分钟

### 解决的核心问题
- ❌ **修复前**：WiFi中断后切换到AP模式，即使WiFi网络恢复也不会自动重连
- ✅ **修复后**：AP模式下定期检测并尝试重连已知WiFi网络，网络恢复后自动连接
- 🔄 **逻辑改进**：移除了`!isAPMode`的限制条件，允许AP模式下进行智能重连

### 新增功能详情
- **统一重连方法**：新增`attemptWiFiReconnection()`方法统一处理重连逻辑
- **配置检查方法**：新增`hasStoredWiFiConfig()`方法检查是否有保存的WiFi配置
- **智能模式切换**：WiFi重连成功后自动关闭AP模式，切换回STA模式
- **状态管理优化**：完善重连计数器和状态标记管理

### 重连策略详情
- **STA模式下连接丢失**：立即尝试重连
- **AP模式下有WiFi配置**：每2分钟尝试重连，最多连续10次（总计20分钟）
- **AP模式下无WiFi配置**：不进行重连尝试，等待用户配置
- **达到最大重连次数**：等待10分钟后重新开始新一轮尝试

### 技术改进
- 修改`WiFiManager::wifiTask()`方法，重构WiFi连接状态检查逻辑
- 优化时间管理，使用`millis()`进行精确的时间间隔控制
- 增强错误处理和状态反馈机制
- 完善日志输出，提供详细的重连状态信息

## 🔄 v7.5.14 版本更新 - 修复天气和定位API密钥同步问题

**最新更新（v7.5.14）**：修复了天气设置页面保存高德API密钥后，定位功能仍然提示"定位失败"的问题。解决了WeatherManager和LocationManager之间API密钥不同步导致的定位服务无法使用的bug。

### v7.5.14 关键修复
- 🔧 **API密钥同步修复**：修改天气设置页面API密钥保存逻辑，同时更新WeatherManager和LocationManager
- 📡 **定位服务修复**：解决首次保存API密钥后点击"获取定位数据"按钮仍提示定位失败的问题  
- ⚡ **一致性保障**：确保天气和定位管理器的API密钥始终保持同步
- 🎯 **用户体验优化**：API密钥保存成功后，定位功能立即可用，无需额外配置

### 修复的问题
- ❌ **修复前**：天气设置页面保存API密钥后，LocationManager仍使用空密钥，导致定位失败
- ✅ **修复后**：保存API密钥时同步更新两个管理器，确保定位功能正常工作
- 🔄 **逻辑改进**：用户只需在天气设置页面保存一次API密钥，即可同时启用天气和定位功能

### 技术改进
- 修改`WebServerManager::handleSetWeatherApiKey()`方法，添加LocationManager API密钥同步逻辑
- 增强错误处理机制，提供详细的成功/失败状态反馈
- 优化用户反馈信息，明确显示天气和定位功能的启用状态

## 🔄 v7.5.13 版本更新 - 修复天气API密钥保存逻辑问题

**最新更新（v7.5.13）**：修复了系统首次使用时的关键逻辑问题，解决保存高德API密钥时的循环依赖问题，优化首次配置流程。

### v7.5.13 关键修复
- 🔧 **循环依赖修复**：解决保存高德API密钥时同时保存天气区域导致的失败问题
- 🗂️ **配置分离保存**：新增`saveApiKeyOnly()`方法，API密钥与城市信息分别保存
- 🎯 **智能城市配置**：新增`setCityInfo()`方法，专门处理城市信息配置
- 📡 **定位集成优化**：修改Web服务器定位逻辑，使用专门的城市信息保存方法
- ⚡ **首次配置优化**：优化首次使用流程：保存API密钥 → IP定位 → 自动配置天气区域

### 修复的问题
- ❌ **修复前**：保存API密钥时尝试同时保存空的城市信息，导致保存失败
- ✅ **修复后**：API密钥独立保存，城市信息通过定位获取后单独保存
- 🔄 **逻辑优化**：避免了需要高德key才能获取天气区域，但保存高德key又需要天气区域的循环依赖

### 新增方法
- `WeatherManager::saveApiKeyOnly()` - 只保存API密钥配置
- `WeatherManager::saveCityConfig()` - 只保存城市信息配置  
- `WeatherManager::setCityInfo()` - 设置并保存城市代码和名称

## 🔄 v7.5.12 版本更新 - 版本号更新

**更新（v7.5.12）**：版本号统一更新到v7.5.12，保持项目版本管理的一致性。

## 🔄 v7.4.8 版本更新 - 集成定位管理系统

**更新（v7.4.8）**：完成定位管理器的集成和天气设置页面的定位功能实现。新增"获取定位数据"按钮，用户可以通过高德地图定位API获取当前位置信息，并自动更新天气配置的城市数据，实现位置数据与天气功能的深度整合。

### v7.4.8 关键更新
- 🌍 **定位管理器集成**：将LocationManager完全集成到主程序和Web服务器中
- 🎯 **智能定位按钮**：在天气设置页面新增"获取定位数据"按钮，支持一键获取位置信息
- 🔗 **自动数据同步**：定位成功后自动更新天气配置中的城市代码和位置信息
- 📡 **API接口完善**：新增定位相关的HTTP API接口，支持前后端数据交互
- 💾 **配置存储优化**：完整的位置信息自动保存到NVS配置存储中
- 🎨 **用户界面增强**：为定位按钮设计美观的动画效果和加载状态显示
- ⚡ **实时更新**：定位完成后页面自动刷新天气配置和天气数据

### 新增功能详情
- **定位获取**：支持通过高德地图IP定位API获取省市区详细信息
- **自动配置**：定位成功后自动设置天气查询的城市代码
- **状态反馈**：完整的加载状态、成功提示和错误处理机制
- **配置持久化**：位置数据自动保存，下次启动时保持配置

### API接口新增
- `GET /api/location/data` - 获取当前定位数据
- `POST /api/location/api-key` - 设置定位API密钥
- `POST /api/location/locate` - 执行定位并更新天气配置

## 🔄 v7.4.7 版本更新 - 天气页面进一步简化

**更新（v7.4.7）**：进一步简化天气设置页面，移除统计数据显示功能。现在天气设置页面专注于核心功能：API配置、定位信息显示、更新设置和操作功能，大幅减少页面复杂度，提升用户体验。

### v7.4.7 关键简化
- 🗂️ **统计移除**：完全删除天气统计信息显示区域，减少页面视觉噪音
- 📊 **代码清理**：移除统计相关的HTML、JavaScript函数和CSS样式
- ⚡ **性能提升**：减少不必要的API调用和DOM更新，提高页面响应速度
- 🎯 **功能聚焦**：页面现在专注于四个核心区域：天气显示、API配置、定位信息、更新设置
- 🧹 **轻量化**：删除`loadWeatherStats()`、`updateStatsDisplay()`等统计相关函数
- 📱 **界面简洁**：去除统计卡片后，页面布局更加简洁明了
...
## 🔄 v7.4.6 版本更新 - 天气设置简化优化

**最新更新（v7.4.6）**：简化天气设置页面，移除手动地区选择功能，现在仅显示当前定位的省市信息和城市编码。这一改进使天气设置更加简洁直观，减少用户操作复杂度，专注于展示核心定位信息。

### v7.4.6 关键改进
- 🎯 **设置页面简化**：移除复杂的省市区三级联动选择器，简化用户界面操作流程
- 📍 **定位信息展示**：优化为仅显示当前定位的省市名称和对应城市编码
- 🗂️ **代码优化**：删除不再需要的JavaScript函数和CSS样式，提高页面加载效率
- 🎨 **界面重设计**：采用新的定位信息显示卡片设计，视觉更加清晰
- ⚡ **功能聚焦**：专注于核心定位信息展示，去除冗余的手动选择功能
- 🧹 **代码清理**：移除locationData相关处理逻辑，简化代码结构

### 界面变化说明
- **之前**：复杂的省市区三级联动选择器 + 手动城市设置保存
- **现在**：简洁的当前定位信息展示卡片
- **优势**：减少用户操作步骤，界面更加直观简洁

## 🔄 v7.4.5 版本更新 - 天气位置数据扩展

**最新更新（v7.4.5）**：新增湖南省和湖北省天气位置数据，大幅扩展天气服务覆盖范围。完善的省市区三级联动选择器现在支持更多城市和区县，为用户提供更精准的天气信息服务。

### v7.4.5 关键功能
- 🌦️ **湖南省位置数据**：新增湖南省全部14个地级市的天气位置数据，包含长沙、株洲、湘潭、衡阳、邵阳、岳阳、常德、张家界、益阳、郴州、永州、怀化、娄底、湘西州等
- 🌦️ **湖北省位置数据**：新增湖北省全部17个地级市的天气位置数据，包含武汉、黄石、十堰、宜昌、襄阳、鄂州、荆门、孝感、荆州、黄冈、咸宁、随州、恩施州、仙桃、潜江、天门、神农架等  
- 📍 **区县级别支持**：每个城市下包含完整的区县数据，支持精确到区县级别的天气信息获取
- 🎯 **三级联动优化**：省市区三级联动选择器体验优化，支持级联选择和实时代码生成
- 🌍 **覆盖范围扩展**：天气服务现已支持全国更多地区，满足不同地区用户需求

## 🔄 v7.4.4 版本更新 - 服务器OTA进度更新修复

**更新（v7.4.4）**：修复了服务器OTA升级时进度条不更新的关键问题。在服务器OTA下载循环中添加了`updateScreenDisplay()`调用，确保每秒更新屏幕上的进度显示，现在服务器OTA和本地文件OTA都能正常显示实时进度更新。

### v7.4.4 关键修复
- 🔧 **进度显示修复**：解决服务器OTA升级时进度条不更新的问题  
- 📈 **实时进度更新**：服务器OTA现在每秒正确更新屏幕上的进度百分比和下载量
- 🛡️ **统一更新机制**：服务器OTA和本地文件OTA现在使用相同的进度更新逻辑
- ⚡ **用户体验提升**：用户可以实时看到OTA升级的进度，提供更好的反馈
- 🔍 **代码一致性**：在下载循环中添加`updateScreenDisplay()`调用，保持与本地OTA的一致性

### 问题根因分析
- **本地文件OTA**：每次`writeOTAData()`调用时通过`updateStatus()`更新屏幕显示
- **服务器OTA**：下载循环中只打印控制台进度，没有调用屏幕更新方法
- **修复方案**：在服务器OTA下载循环的进度更新处添加`updateScreenDisplay()`调用

## 🔄 v7.4.3 版本更新 - 服务器OTA页面显示修复

**更新（v7.4.3）**：修复了服务器OTA升级时不切换到OTA进度页面的关键问题。通过在`downloadAndWriteFirmware()`函数中添加`startOTADisplay()`调用，确保服务器OTA与本地文件OTA拥有一致的页面显示行为，提升用户体验和系统可靠性。

### v7.4.3 关键修复
- 🔧 **服务器OTA页面修复**：解决服务器OTA升级时不显示进度页面的问题
- 📱 **统一显示逻辑**：本地文件OTA和服务器OTA现在拥有一致的页面切换行为
- 🛡️ **流程完整性**：确保服务器OTA流程包含所有必要的UI反馈
- ⚡ **用户体验提升**：服务器OTA现在也能正确显示升级进度和状态
- 🔍 **调试日志增强**：在服务器OTA页面显示启动时添加调试信息
- 💪 **代码一致性**：统一了两种OTA方式的DisplayManager调用逻辑

### 问题根因分析
- **本地文件OTA**：通过`beginOTA()`→`startOTADisplay()`正确显示页面
- **服务器OTA**：绕过了`beginOTA()`，直接使用`Update.begin()`，缺少页面显示调用
- **修复方案**：在`downloadAndWriteFirmware()`中添加`startOTADisplay(true)`调用

## 🔄 v7.4.2 版本更新 - 小电拼按钮文案优化

**更新（v7.4.2）**：进一步优化小电拼设置页面的用户体验，将"扫描cp02服务器"按钮及相关文案全面更改为"扫描小电拼"，使界面语言更加统一友好。更新涵盖Web界面按钮、扫描结果显示、JavaScript提示信息以及后端日志，确保整个扫描功能的命名保持一致性。

### v7.4.2 关键更新
- 🔘 **扫描按钮优化**：主要扫描按钮从"扫描cp02服务器"更改为"扫描小电拼"
- 📋 **扫描结果标题**：扫描结果区域标题更新为"小电拼扫描结果"
- 💭 **提示信息统一**：所有相关提示文案统一使用"小电拼"命名
- 🔔 **JavaScript消息**：扫描成功、失败、选择设备等提示信息全部更新
- 📝 **后端日志优化**：后端处理和日志信息同步更新为"小电拼"
- 🎯 **用户体验提升**：界面语言更加统一，用户理解更加直观

## 🔄 v7.4.1 版本更新 - 小电拼设置界面优化

**最新更新（v7.4.1）**：优化Web界面用户体验，将"服务器设置"按钮名称更改为"小电拼设置"，使功能名称更加直观易懂。更新涉及主页面按钮、设置页面标题、副标题以及相关的JavaScript提示信息，为用户提供一致的界面体验。

### v7.4.1 关键更新
- 🏷️ **界面文案优化**：主页面按钮从"服务器设置"更改为"小电拼设置"
- 📝 **页面标题统一**：设置页面标题和副标题同步更新为"小电拼设置"
- 💬 **提示信息一致**：JavaScript状态消息更新为"小电拼设置已就绪"
- 🔄 **用户体验提升**：功能名称更加直观，便于用户理解和使用
- ✨ **界面一致性**：确保所有相关文本保持统一的命名规范

## 🔄 v7.4.0 版本更新 - OTA版本对比功能

**最新更新（v7.4.0）**：在服务器OTA升级功能中新增智能版本对比机制。系统在执行OTA升级前会自动检查当前设备版本与服务器版本的差异，如果版本相同则提示"已是最新版本，无需升级"，如果当前版本更高则提示"当前版本更新，无需升级"，只有当服务器版本更高时才允许升级操作。前端界面根据版本比较结果动态控制升级按钮状态，为用户提供直观的版本状态反馈。

### v7.4.0 关键特性
- 🔍 **智能版本对比**：服务器OTA升级前自动检查版本差异，支持语义化版本比较
- 🚫 **防止重复升级**：相同版本或当前版本更新时阻止升级，避免不必要的操作
- 📊 **版本状态显示**：直观展示当前版本与服务器版本的比较结果和升级建议
- 🎯 **智能UI控制**：根据版本比较结果动态启用/禁用升级按钮，优化用户体验
- 💡 **友好提示信息**：为用户提供明确的版本状态说明和升级建议
- ⚡ **版本解析增强**：完善的版本号解析逻辑，支持"v6.7.2"格式的版本比较
